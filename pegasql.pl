...
## LICENSE   ###############################################################################################################################################################################
#
#   This software is Copyright (c) Synchronize Team Hacker
#   Tool coded by Pegasus member of Synchronize Team Hacker.
#
...
## INTRODUCTION #############################################################################################################################################################################
#
#   This script is Copyright (c) Synchronize Team Hacker
#   Tool coded by Pegasus.
#   Script name: Pegasql.
#   Codename: Muhammad Sobri Maulana, S.Ked, CEH.
...
#
#   [:] AUTOR:        Synchronize Team Hacker
#   [:] FB:           https://facebook.com/muhammadsobrimaulana
#   [:] Twitter:      https://twitter.com/muhsobrimaulan1
#   [:] Pastebin      http://http://pastebin.com/u/pegasql
#   [:] GIT:          https://github.com/pegasql
#   [:] YOUTUBE:      http://youtube.com/c/pegasql
#
############################################################################################################################################################################################
## REQUIREMENT  ############################################################################################################################################################################
# Requiered libreries
...
## BANNER
sub banner {
  my @ESLOGAN=("Indonesia!", "Pegasus Hacker", "Synchronize", "Database !","Muhammad Sobri Maulana");
my @LOGO = ("
  \|  _ \   | ____| / ___|     / \   / ___| | | | | / ___| 
  \| |_) | |  _|   | |  _    / _ \   \___ \  | | | | \___ \ 
  \|  __/  | |___  | |_| |  / ___ \   ___) | | |_| |  ___) |
  \|_|     |_____|  \___ _| /_/   \_\ |____/   \___/  |____/  "
  );
...
## CHECK VERSION
sub checkVersion {
  my $URL = 'https://github.com/AlisamTechnology/pegasql/blob/master/version.log';
  my $request = HTTP::Request->new('GET', $URL);
  my $response = $ua->request($request);
  ...
  if ($gitversion eq $existantVersion) {
	...
	if ($resp =~ /$yes/) {
      my $URL = "https://raw.githubusercontent.com/AlisamTechnology/pegasql/master/pegasql.pl";
      my $request = HTTP::Request->new('GET', $URL);
      my $response = $ua->request($request);
      my $html = $response->content;
	  if ($response->is_success) {
	    if (-e '/usr/share/doc/pegasql/pegasql.pl') {
		  unlink '/usr/share/doc/pegasql/pegasql.pl';
		  open (FILE, '>>/usr/share/doc/pegasql/pegasql.pl');
          print FILE $response->content;
          close (FILE);
		}else{
		  unlink 'pegasql.pl';
		  open (FILE, '>>pegasql.pl');
          print FILE $response->content;
          close (FILE);
	    }
		...
		sleep(2);
	    if (-e '/usr/share/doc/pegasql/pegasql.pl') {
	      system("chmod +x /usr/share/doc/pegasql/pegasql.pl | pegasql");
		}else{
		  system("chmod +x pegasql.pl | perl ./pegasql.pl");
		  exit();
		}	     
      }else{
...
sub osinfo {
  ...
  print "\033[0;36mSynchronize Team Hacker SCANNER\n";
  print "\033[1;33m[:] TOOL:: ";
  print "\033[0;36mpegasql $existantVersion\n";
...
sub advise {
  ...
  if ($lalwmali =~ m/\/usr\/share\/doc/i) {
    print "\033[0;33m[!] Usage: pegasql <option> \n";
    print "[!] Help: pegasql [--help | -h]\n";
  }else{
...
sub resultsTOcommand {
  if (defined $command) {
    ...
    use File::Copy qw(copy);
	copy $Bin.'/pegasql.txt', $Bin.'/aTsearch.txt';
  }  
}
############################################################################################################################################################################################
############################################################################################################################################################################################
...
sub desclaimer {
  ...
  print "\033[0;37m Tools milik Pegasus yang dapat melacak dan menemukan website                \033[1;37m[:]\n\033[1;37m[:]";  
  print "\033[0;37m      yang memiliki kelemahan sistem berdasarkan dork                        \033[1;37m[:]\n\033[1;37m[:]";
  print "\033[0;37m        Created by dr. Muhammad Sobri Maulana, CEH !!                        \033[1;37m[:]\n";               
  ...
  if (-e $list){
    ...
    my %seen = ();
    {
      ...
      while(<>){
        ...
        print;
        close (TEXT);
		unlink $Bin."/pegasearch.txt.bac";
      }
    }   
    my $count2=0; 
    open (URL, $Bin.'/pegasearch.txt');
    while (my $URL = <URL>) {
	  chomp $URL;                  
	  $count2++;
	...
	if (-e $Bin.'/pegasql.txt'){ unlink $Bin.'/pegasql.txt'};
  }else{
    deleteLists();
    negative();
	exit();
...
############################################################################################################################################################################################
############################################################################################################################################################################################
## Copy@right Synchronize Team Hacker Team
## 
############################################################################################################################################################################################
############################################################################################################################################################################################
